...
def train_cifar_model(
    ...
    if optimizer_type == "Adam":
        ...
        optimizer = ivon.IVON(model.parameters(), lr=lr, ess=len(train_loader), weight_decay=1e-3, beta1=0.9, beta2=0.99)
        train_losses = []
        test_accuracies = []
        for epoch in range(num_epochs):
            model.train()
                ...
                for _ in range(64):
                    with optimizer.sampled_params():
                        if model_type == "batchensemble" or model_type == "batchensemble_complex":
                            ...
                            outputs = model(images)
                        sampled_probs.append(F.softmax(outputs, dim=1))
                prob = torch.mean(torch.stack(sampled_probs), dim=0)
                _, pred = prob.max(1)
                test_loss -= torch.sum(torch.log(prob.clamp(min=1e-6)) * F.one_hot(labels, 10), dim=1).mean()
...
def train_with_params(
        ...
        optimizer_type: str = typer.Option("Adam", help="Type of optimizer")
):
    ...
    for model_type in model_types:
        ...
        result = {
            ...
            "cnn_train_losses": cnn_train_losses,
            "cnn_test_accuracies": cnn_test_accuracies,
            "optimizer_type": optimizer_type
        }
...
def run_experiments():
    ...
    alpha_list = [-0.1]
    gamma_list = [-0.1]
    ensemble_size_list = [10]
    seed_list = [42]
    epochs_list = [15]
    batch_list = [250]
    lr_list = [0.0002]
    optimizer_list = ["Adam", "IVON"]
    ...
    adam_file = os.path.join(output_folder, "adam_results.csv")
    ivon_file = os.path.join(output_folder, "ivon_results.csv")
    ...
    for optimizer_type in optimizer_list:
        if optimizer_type == "Adam":
            print(f"Calculation with Adam")
            results = train_with_params(seed=42, ensemble_size=10, alpha_init=-0.1, gamma_init=-0.1, num_epochs=15, batch_size=250, lr=0.0002, device=device, optimizer_type=optimizer_type)
            save_results_to_csv(results, adam_file)
        elif optimizer_type == "IVON":
            print(f"Calculation with IVON")
            results = train_with_params(seed=42, ensemble_size=10, alpha_init=-0.1, gamma_init=-0.1, num_epochs=15, batch_size=250, lr=0.0002, device=device, optimizer_type=optimizer_type)
            save_results_to_csv(results, ivon_file)
    # print(f"Calculation with IVON")
    # results = train_with_params(seed=42, ensemble_size=1, alpha_init=-0.1, gamma_init=-0.1, num_epochs=15, batch_size=256, lr=0.0002, device=device, optimizer_type="IVON")
    # save_results_to_csv(results, ivon_file)


sys.argv = ["IvonAdam.py"]
