...
def run_experiments():
    ...
    optimizer_file = os.path.join(output_folder, "sgd_results4.csv")  ###

    # test optimizer
    print(f"Next calculation with differences in alpha and gamma: alpha={alpha} and gamma={gamma}")
    results = train_with_params(seed=42, ensemble_size=ensemble_size, alpha_init=alpha, gamma_init=gamma, num_epochs=15,
                                batch_size=256, lr=lr, device=device, optimizer_type=optimizer_type)
