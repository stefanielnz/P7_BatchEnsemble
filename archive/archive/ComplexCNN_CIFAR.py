...
class BatchEnsembleCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))   # CIFAR [batch_size * ensemble_size, 32, 32, 32]
        x = self.pool(x)                         # CIFAR [batch_size * ensemble_size, 32, 17, 17]
        x = self.relu(self.bn2(self.conv2(x)))   # CIFAR [batch_size * ensemble_size, 64, 17, 17]
        x = self.pool(x)                         # CIFAR [batch_size * ensemble_size, 64, 8, 8]
        x = x.view(x.size(0), -1)                # CIFAR [batch_size * ensemble_size, 64*8*8] 
        x = self.fc(x)                           # CIFAR [batch_size * ensemble_size, 10]
        return x
...
def main():
    ...
    cnn_train_losses = {}
    cnn_test_accuracies = {}
    model_types = ["complex"]#, "batchensemble"]
    for model_type in model_types:
        if model_type == "complex":
            ...
            raise ValueError("Invalid model_type. Choose 'complex' or 'batchensemble'")
